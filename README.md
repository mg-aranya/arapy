...
- added property-based fuzz tests with `hypothesis` for parser, masking, and query-serialization invariants
- documented that fuzz coverage is installed via `pip install -e '.[dev]'`
- ignored local `.hypothesis/` state generated by fuzz runs
